...
class VivitTubeletEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        # print(f"from VivitTubeletEmbeddings tubelet_size = {self.patch_size}")# (2,16,16)
        # print(f"from VivitTubeletEmbeddings self.image_size = {self.image_size}")#(224)
        # print(f"from VivitTubeletEmbeddings self.num_frames  = {self.num_frames}")#(32)
        self.num_patches = (
            (self.image_size // self.patch_size[2])
            * (self.image_size // self.patch_size[1])
            * (self.num_frames // self.patch_size[0])
        )
        # print(f"from VivitTubeletEmbeddings self.num_patches = {self.num_patches}")#before 3136 after 
        self.num_patches += 100  
        # print(f" self.num_patches after prompts= {self.num_patches}")# after 
        self.embed_dim = config.hidden_size
        # print(f"from VivitTubeletEmbeddings self.embed_dim  = {self.embed_dim}")
        # print(f"from VivitTubeletEmbeddings config.hidden_size  = {config.hidden_size}")
        # print(f"from VivitTubeletEmbeddings config.num_channels  = {config.num_channels}")
        self.projection = nn.Conv3d(
            config.num_channels, config.hidden_size, kernel_size=config.tubelet_size, stride=config.tubelet_size
        )
        ##### Prompting info ##### 
        prompt_dim = config.hidden_size
        val = math.sqrt(6. / float(3 * reduce(mul, self.patch_size, 1) + prompt_dim))  # noqa
    ...
    def incorporate_prompt(self, x):
        ...
        Batch = x.shape[0]
        # print(f"self.prompt_embeddings = {self.prompt_embeddings.data.shape}")
        # after CLS token, all before image patches
        # x = self.embeddings(x)  # (batch_size, 1 + n_patches, hidden_dim)
        # logger.info(f"size of patches without prompt = {x.size()}") # torch.Size([19, 197, 768])
        x = torch.cat((
                # x[:, :1, :],#cls
                self.prompt_embeddings.expand(Batch, -1, -1),#prompts -1 is to remain that dim unchanged
                x#patches
            ), dim=1)
        # logger.info(f"prompts embedding size {x.size()}") #(batch_size,patches+Num_Tokens,hidden_dim) = (21,1+196+100,768)
    ...
    def forward(self, pixel_values, interpolate_pos_encoding: bool = False):
        ...
        # print(f"patch shape after conv3d is {x.shape}")#([1, 768, 66, 14, 14])
        # out_batch_size, out_num_channels, out_num_frames, out_height, out_width = x.shape
        x = self.projection(pixel_values).flatten(2).transpose(1, 2)#([1, 3136, 768])hout cls
        # print(f"patch shape after conv3d projection is {x.shape}")#
        prompted_embeddings = self.incorporate_prompt(x)
        # logger.info(f"prompts embedding size {prompted_embeddings.size()}") #(batch_size,patches+Num_Tokens,hidden_dim) = (21,1+196+100,768)
